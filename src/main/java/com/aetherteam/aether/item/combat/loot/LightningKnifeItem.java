...
public class LightningKnifeItem extends Item implements ProjectileItem {
     ...
     * {@link com.aetherteam.aether.item.combat.abilities.weapon.WeaponAbilities#lightningTracking} prevents the attacker from being injured by the lightning.
     *
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            if (!player.getAbilities().instabuild && EnchantmentHelper.getItemEnchantmentLevel(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.INFINITY), heldStack) == 0) { // Note: Lightning knives can't be enchanted with Infinity in survival, but we still implement the behavior.
                heldStack.shrink(1);
            }
